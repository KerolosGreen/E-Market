...
<nav class="fixed top-0 z-50 w-full bg-white border-b border-gray-200 dark:bg-gray-800 dark:border-gray-700">
      ...
      <aside id="logo-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen pt-20 transition-transform -translate-x-full bg-white border-r border-gray-200 sm:translate-x-0 dark:bg-gray-800 dark:border-gray-700" aria-label="Sidebar">
         <div class="h-full px-3 pb-4 overflow-y-auto bg-white dark:bg-gray-800">
            <ul class="space-y-2 font-medium">
               <li>
                  <a href="{{route('admin.dashboard')}}" class="active flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
      ...
      <div class="p-4 sm:ml-64">
         <div class="p-4 rounded-lg bg-gray-100 dark:border-gray-700 mt-14">
            <!--First DIV-->
            <div class="grid grid-cols-1 gap-4 mb-4 sm:grid-cols-1 md:grid-cols-3">

               <div class="flex items-center justify-start p-2 h-24 rounded-lg  bg-white dark:bg-gray-800">
                <div class="users-dashboard-icon">
                <i class="fa-solid fa-user" style="color: #ffffff;"></i>
                </div>
                <div class="users-dashboard dark:text-white">
                <p class="dark:text-gray-300">Total Users</p>
                <h4 style="font-weight: 500;">{{$users}}</h4>
                </div> 
               </div>

               <div class="flex items-center justify-start p-2 h-24 rounded-lg bg-white dark:bg-gray-800">
                <div class="orders-dashboard-icon">
                <i class="fas fa-motorcycle" style="color: #ffffff;"></i>
                </div>
                <div class="users-dashboard dark:text-white">
                <p class="dark:text-gray-300">Orders Placed</p>
                <h4 style="font-weight: 500;">{{$orders_count}}</h4>
                </div> 
               </div>

               <div class="flex items-center justify-start p-2 h-24 rounded-lg  bg-white dark:bg-gray-800">
                <div class="products-dashboard-icon">
                    <i class="fas fa-shopping-bag" style="color: #ffffff;"></i>
                </div>
                <div class="users-dashboard dark:text-white">
                <p class="dark:text-gray-300">Current Products</p>
                <h4 style="font-weight: 500;">{{$products}}</h4>
                </div> 
               </div>
            ...
            <!--First DIV-->
            <!--Second DIV-->
            <h4 class="recent-orders-dashboard">Recent Orders</h4>
            <div class="flex items-center justify-center h-50 mb-4 rounded-lg bg-gray-50 dark:bg-gray-800 tableparentdiv">
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 latestordersdashboard">
        ...
        <tbody>
         ...
         <!--Three Recent Orders Rows-->
           <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
            ...
            <td class="px-6 py-4">
               @php $sum=0; foreach($order->orderitem as $item){$sum += $item->product->price * $item->quantity;}echo $sum;@endphp
               
            </td>
            <td class="px-6 py-4">
            {{$order->payment==NULL?'Cash OD':'Visa - '.substr($order->payment->cc,-4)}}
            </td>
            <td class="px-6 py-4">
                Delivered
            </td>
            ...
            @endforeach
            @else
            <tr><td class="px-6 py-4 text-center" colspan="6"><h4>No Orders Yet</h4></td></tr>
            @endif
        </tbody>
    </table>
         ...
         <!--Second DIV-->
         <!--Third DIV-->
         <h4 class="font-medium text-gray-900 dark:text-gray-900 m-2 text-base">Edit Website Banner</h4>
         <div class="flex items-center justify-between h-48 mb-4 p-5 rounded-lg bg-white dark:bg-gray-800 bannerdashboard">
            <div class="flex flex-col items-center">
               <h4 class="text-gray-900 dark:text-gray-300 font-medium">Current Banner</h4>
               @if($banner!=null)
               <img width="300px" style="max-height: 150px;" class="rounded-xl" src="/images/{{$banner->image}}">
            @else
            <h4 class="text-white">There Is No Image Now</h4>
            @endif
            </div>
            <form method="POST" action="{{route('setbanner')}}" enctype="multipart/form-data">
               @csrf
                  <input type="file" name="image" class="dark:text-white w-1/6">
                  <button name="SET" class="text-white bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2">SET</button>
               </form>
         </div>
         <!--Third DIV-->
      </div>
